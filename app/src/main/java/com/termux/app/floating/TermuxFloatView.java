...
public class TermuxFloatView extends LinearLayout {
    ...
    void updateLongPressMode(boolean newValue) {
        ...
        if (newValue && !mFloatingBubbleManager.isMinimized())
            Logger.showToast(getContext(), getContext().getString(R.string.float_long_press_instruction), false);
    }
    ...
    private void exit() {
        Intent exitIntent = new Intent(getContext(), TermuxFloatService.class).setAction(TermuxConstants.TERMUX_APP.TERMUX_SERVICE.ACTION_STOP_SERVICE);
